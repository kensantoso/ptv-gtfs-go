...
func (p Policy) transferOverride(from, to string) (time.Duration, bool) {
	return lookupPair(p.TransferTimes, from, to)
}

// lookupPair reads a stated time for a change, trying the reverse direction
// second: crossing a footbridge takes the same time whichever way you walk, so
// only one direction need be given, and an explicit reverse still wins.
func lookupPair(m map[StopPair]time.Duration, from, to string) (time.Duration, bool) {
	if len(m) == 0 {
		return 0, false
	}
	if d, ok := m[StopPair{From: from, To: to}]; ok {
		return d, true
	}
	d, ok := m[StopPair{From: to, To: from}]
	return d, ok
}
