...
type PlanRequest struct {
	...
	MaxWalk float64
	// TransferTimes overrides how long a change takes, for this request only.
	// Same shape as [Policy.TransferTimes] and consulted ahead of it, so a
	// caller holding one shared [Index] can still answer per-user: a browser
	// keeps its own figures and sends them, without a Policy per session.
...
func (ix *Index) catchable(ctx context.Context, leg1 Leg, onward []Journey,
	ic interchange, walk float64, buffer time.Duration,
	stated map[StopPair]time.Duration) (*Journey, time.Duration, error) {
	...
	for i := range onward {
		...
		need, err := ix.transferTime(ctx, leg1.ToStop, cand.Legs[0].FromStop, stated)
		if err != nil {
			return nil, 0, err
		}
...
func (ix *Index) journeysWithOneChange(ctx context.Context, from, to []string,
	...
	for _, station := range stations {
		...
		for _, f := range firsts {
			leg1 := f.Legs[0]
			best, need, err := ix.catchable(ctx, leg1, onward, ic, walk, req.TransferBuffer, req.TransferTimes)
			if err != nil {
				return nil, err
			}
			if best == nil {
