...
func (ix *Index) TransferTime(ctx context.Context, from, to string) (time.Duration, error) {
	return ix.transferTime(ctx, from, to, nil)
}

// transferTime is TransferTime with a per-request override layer on top.
//
// Four sources become five: the request, then the policy, then the feed's two,
// then the default. A time the caller stated wins outright and is not floored by
// the default, because someone saying ninety seconds has measured it.
func (ix *Index) transferTime(ctx context.Context, from, to string, req map[StopPair]time.Duration) (time.Duration, error) {
	if d, ok := lookupPair(req, from, to); ok {
		return d, nil
	}
