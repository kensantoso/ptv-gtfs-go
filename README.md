...
```go
ix.StopName(ctx, stopID)                          // one name
...
workable connections get discarded. `TransferTimes` overrides it per pair of
stops, ahead of every other source.
...
Ids are stop ids, which for trains are platforms rather than stations — use
[Index.StopIDsForStation] to list them. Lookup is symmetric, so one entry covers
both directions unless the reverse is also given, which is worth doing where
stairs down are not stairs up.
