...
func platformsOf(ctx context.Context, db *sql.DB, station string) (map[string]string, error) {
	// Must actually have numbered platforms, or "Richmond" resolves to the bus
	// stop "Richmond St/Fulton Rd" — a shorter name, no parent, and no platforms
	// to transfer between. Exact name still wins, so "Richmond" is not East
	// Richmond.
	var id string
	err := db.QueryRowContext(ctx, `
		SELECT s.stop_id FROM stop s
		WHERE (s.name = ? COLLATE NOCASE OR s.name LIKE ? COLLATE NOCASE)
		  AND COALESCE(s.parent,'') = ''
		  AND EXISTS (SELECT 1 FROM stop c
		              WHERE c.parent = s.stop_id AND COALESCE(c.platform,'') != '')
		ORDER BY CASE WHEN s.name = ? COLLATE NOCASE THEN 0 ELSE 1 END, LENGTH(s.name)
